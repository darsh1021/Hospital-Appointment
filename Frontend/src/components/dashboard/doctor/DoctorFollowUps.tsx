...
import { fetchDoctorFollowups } from "../../../Features/doctor/doctorSlice";
...
type FollowUpPatient = {
  ...
  tokenBooked: string | number | null
  diagnosis: string
}

const statusConfig: Record<FollowUpStatus, { label: string; bg: string; text: string; icon: ElementType }> = {
...
const DoctorFollowUps = () => {
  ...
  const { followups, loading, error } = useAppSelector(state => state.doctor);

  useEffect(() => {
    dispatch(fetchDoctorFollowups());
  }, [dispatch]);

  const mappedFollowUps: FollowUpPatient[] = followups.map(f => ({
    id: f.id,
    name: f.patient_name,
    age: "N/A",
    lastVisit: new Date(f.appointment_date).toLocaleDateString(),
    scheduledFollowUp: "N/A",
    status: "not-booked" as FollowUpStatus,
    tokenBooked: f.token_number,
    diagnosis: f.symptoms || "N/A",
  }));

  const overdue = mappedFollowUps.filter(p => p.status === "overdue")
  const booked = mappedFollowUps.filter(p => p.status === "booked")
  const notBooked = mappedFollowUps.filter(p => p.status === "not-booked")
            ...
            return (
              <div key={patient.id} className="flex flex-col gap-4 p-6 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-start gap-4">
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-lg bg-[#fafafa] border border-[#ebebeb] dark:border-white/10 dark:bg-white/5">
  ...
  if (loading) return <div className="p-6 text-[#888888]">Loading follow-ups...</div>;
  ...
  return (
    <div className="space-y-8 max-w-5xl">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 rounded-full border border-[#ebebeb] bg-[#fafafa] px-4 py-2 dark:border-white/10 dark:bg-[#171717]">
            ...
            <span className="text-[13px] text-[#888888]">{mappedFollowUps.length} scheduled</span>
          </div>
        </div>
      </div>
      ...
      {/* Overdue alert */}
      {overdue.length > 0 && (
        <div className="flex items-center gap-3 rounded-xl border border-[#f7d4d6] bg-[#f7d4d6]/40 px-5 py-4 dark:border-[#ee0000]/20 dark:bg-[#ee0000]/10">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={s.label} className={`flex flex-col gap-1 rounded-xl border border-[#ebebeb] p-5 dark:border-white/10 ${s.bg}`}>
