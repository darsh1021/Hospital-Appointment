import { ListOrdered, ChevronRight, AlertCircle, UserCheck, UserX, Ticket } from "lucide-react"

import { useEffect } from "react";
import { useAppDispatch, useAppSelector } from "../../../app/store";
import { fetchDoctorQueue, updateAppointmentStatus } from "../../../Features/doctor/doctorSlice";

const statusConfig: Record<string, { label: string; bg: string; text: string }> = {
  "in-consultation": { label: "Serving",   bg: "bg-[#d3e5ff] dark:bg-[#0070f3]/20",   text: "text-[#0761d1] dark:text-[#50e3c2]" },
  "waiting":         { label: "Waiting",   bg: "bg-[#fafafa] dark:bg-white/5",         text: "text-[#888888]" },
  "scheduled":       { label: "Scheduled", bg: "bg-[#fafafa] dark:bg-white/5",         text: "text-[#888888]" },
  "cancelled":       { label: "No-show",   bg: "bg-[#f7d4d6] dark:bg-[#ee0000]/20",   text: "text-[#ee0000] dark:text-[#ff4d4d]" },
  "completed":       { label: "Done",      bg: "bg-[#fafafa] dark:bg-white/5",         text: "text-[#888888]" },
  "recalled":        { label: "Recalled",  bg: "bg-[#fff6d6] dark:bg-[#f5a623]/10", text: "text-[#ab570a] dark:text-[#f5a623]" },
}

const DoctorQueue = () => {
  const dispatch = useAppDispatch();
  const { queue, loading, error } = useAppSelector(state => state.doctor);

  useEffect(() => {
    dispatch(fetchDoctorQueue());
  }, [dispatch]);

  const active = queue.filter(p => ["scheduled", "waiting", "in-consultation"].includes(p.status))
  const past   = queue.filter(p => ["cancelled", "completed"].includes(p.status))
  const hasRecalled = queue.some(p => (p.status as string) === "recalled")
  ...
  if (loading) {
    return <div className="p-6 text-[#888888]">Loading queue...</div>;
  }
  if (error) {
  ...
  return (
    <div className="space-y-8 max-w-5xl">
      ...
      {/* Recalled notice */}
      {hasRecalled && (
        <div className="flex items-center gap-3 rounded-xl border border-[#ffefcf] bg-[#ffefcf]/50 px-5 py-4 dark:border-[#f5a623]/20 dark:bg-[#f5a623]/10">
            ...
            return (
              <div
                ...
                className={`grid grid-cols-1 md:grid-cols-[48px_1fr_160px_90px_90px_140px] gap-4 px-6 py-4 items-center ${
                  patient.status === "in-consultation" ? "bg-[#d3e5ff]/20 dark:bg-[#0070f3]/5" : ""
                }`}
              >
                {/* Token */}
                ...
                {/* Visit type */}
                <span className="text-[13px] text-[#4d4d4d] dark:text-[#888888]">N/A</span>
                {/* Fee */}
                <span className="text-[13px] font-mono text-[#171717] dark:text-white">N/A</span>
                {/* Est wait */}
                ...
                <div className="flex gap-2">
                  {patient.status === "in-consultation" && (
                    <>
                      {/* Done is handled in Current Patient screen, but for consistency if here: */}
                      <button
                        id={`btn-complete-${patient.token_number}`}
                      ...
                      <button
                        id={`btn-noshow-${patient.token_number}`}
                        ...
                        onClick={() => handleUpdateStatus(patient.id, "cancelled")}
                      >
                        <UserX size={12} /> No-show
                      </button>
                    </>
                  )}
                  {(patient.status === "scheduled" || patient.status === "waiting") && (
                    <button
                      id={`btn-call-${patient.token_number}`}
      ...
      <div className="flex flex-col rounded-xl border border-[#ebebeb] bg-white shadow-[0px_2px_2px_#0000000a,0px_8px_16px_-4px_#0000000a] dark:border-white/10 dark:bg-[#0a0a0a]">
        ...
        <div className="flex flex-col divide-y divide-[#ebebeb] dark:divide-white/10">
          {past.map((patient) => {
            const sc = statusConfig[patient.status]
            return (
              <div key={patient.token_number} className="flex items-center justify-between px-6 py-4 opacity-60">
                <div className="flex items-center gap-3">
                  <span className="font-mono text-[13px] text-[#888888]">#{patient.token_number}</span>
                  <div>
                    <p className="text-[14px] font-medium text-[#171717] dark:text-white">{patient.patient_name}</p>
